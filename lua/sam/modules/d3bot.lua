if engine.ActiveGamemode() == "zombiesurvival" then
	...
	function Temp.human(pl)
		...
		pl:StripWeapons()
		pl:StripAmmo()
		Temp.giveHumanLoadout(pl)
		Temp.tryBringToHumans(pl)
	end
	sam.command.set_category("Zombie Survival")

	sam.command.new("human")
		:SetPermission("human", "user")
		:Help("If you're a zombie, you can use this command to instantly respawn as a human with a default loadout.")
		:DisallowConsole()
		:OnExecute(Temp.human)
		:End()
end
sam.command.set_category("D3bot")

local function registerCmd(camelCaseName, access, ...)
	local lowerName = camelCaseName:lower()
	local cmdStr = (access == "superadmin" and "bot" or "") .. lowerName
	local cmd = sam.command.new(cmdStr)
	cmd:SetPermission(cmdStr, access)

	for idx, arg in ipairs{ ... } do
		if istable(arg) then
			if arg.type == "rest" then
				cmd:GetRestArgs(true)
				cmd:AddArg("text", arg.options or {})
			else
				cmd:AddArg(arg.type, arg.options or {})
			end
...
local function registerSuperadminCmd(camelCaseName, ...) registerCmd(camelCaseName, "superadmin", ...) end
local function registerAdminCmd(camelCaseName, ...) registerCmd(camelCaseName, "admin", ...) end

local plsParam = { type = "player", options = {single_target = false} }
local numParam = { type = "number" }
local strParam = { type = "text" }
local strRestParam = { type = "rest" }
local optionalStrParam = { type = "text", options = {optional = true} }
